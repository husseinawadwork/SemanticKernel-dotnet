...
OpenApiToolDefinition parksinformationOpenApiTool = new(
    ...
    spec: BinaryData.FromBytes(File.ReadAllBytes(@"./specs/openapi.json")),
    auth: oaiAuth
);
...
Response<ThreadMessage> userMessageResponse = await client.CreateMessageAsync(
    thread.Id,
    MessageRole.User,
    "My name is Hussein, I want to know the weather in Sydney, and also information from the parks in the city");
